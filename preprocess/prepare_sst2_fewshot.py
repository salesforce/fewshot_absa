    ...
    for shot in shots:
        ...
        for sentiment in data_dict:
            ...
            for d in sentiment_data:
                ...
                sentiment_original = sst2_labels[sentiment]
                d_original = d.split('<|review|>')[-1].split('<|endofreview|>')[0].strip()
                text_original = f"{d_original}\t{sentiment_original}"
        ...
        data_dir = sst2_dir
        save_name_original = os.path.join(resource_dir, data_dir, f"train_{shot}.tsv")
